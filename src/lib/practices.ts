...
export const PRACTICES: Practice[] = [
  ...
  },
  {
    ...
    steps: ['Follow the first instruction. Switch its presentation and ask whether the task has changed.', 'Choose the changed instruction and make a second drawing.', 'Trace the chain: screen, marks, interpretation, hand, paper. Which parts were necessary for the pattern to do anything here?', 'If you want to take this further, trace a bus timetable through drivers, vehicles, stops, roads, and passengers. Who maintains, interprets, and changes it? What happens when the promise on the timetable and the service differ?'],
    note: 'The drawing instructions ask you to act. The timetable also makes a claim about what you can expect. Tracing how marks have an effect does not, by itself, tell you whether a claim is accurate.',
    chapters: [10, 19], fields: [
      ...
      { key: 'transfer', label: 'Optional: how does a bus timetable become an actual journey?', hint: 'Trace the people and things it depends on, who can change it, and what happens when the service differs from the promise.' },
    ],
  },
  {
    ...
    steps: ['Predict one effect of changing one rule. Keep the other settings steady first.', 'Compare harvest with labor, excluded households, and households without a vote. Then try a dry season.', 'Open the assumptions. Propose one different assumption and one claim about fairness that harvest alone cannot settle.', 'If you want to take this further, consider a public library’s opening hours. What do they make possible? Whose schedules fit? What would different hours require of staff, funding, and the people who decide? You can use a library you know or imagine one.'],
    note: 'The numbers come from an invented one-season model. Inspecting its assumptions is part of the exercise. It cannot decide how a real community should govern itself, and its numbers should not be transferred to a library or other real institution.',
    chapters: [9, 19, 20], fields: [
      ...
      { key: 'transfer', label: 'Optional: whose days fit a library’s opening hours?', hint: 'What does the rule make possible? Consider staff and resources the garden model leaves out, and who could revise the hours.', after: true },
